<link href="/FormBuilder/public/css/plugins/chartist/chartist.min.css" rel="stylesheet">

<?php
$percentage = count ($incompleteProjects) / count ($arrCompletedProjects) * 100; // string: 100%
        ...
        <div class="col-lg-4">
            <div class="ibox float-e-margins">
                <div class="ibox-title">
                    ...
                    <h5>Number of projects for user</h5>
                </div>
                <div class="ibox-content">
                    <h1 class="no-margins"><?= count ($arrProjectsForUser) ?></h1>
                </div>
            </div>
        </div>
        <div class="col-lg-4">
        ...
        <div class="col-lg-6">
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content inspinia-timeline">
                    ...
                    <?php
                    foreach ($arrProjectsForUser as $projectId):
                        $stepData = json_decode ($arrAllProjects[$projectId]['step_data'], true);
                        ?>
                        <div class="timeline-item">
                            <div class="row">
                                <div class="col-xs-3 date">
                                    <i class="fa fa-briefcase"></i>
                                    <?= $stepData['job']['dueDate'] ?>
                                    <br>
                                    <small class="text-navy"><?= $stepData['job']['priority'] ?></small>
                                </div>
                                <div class="col-xs-7 content no-top-border">
                                    <p class="m-b-xs"><strong><?= $stepData['job']['name'] ?></strong></p>

                                    <p><?= $stepData['job']['description'] ?></p>
...
<!-- Chartist -->
<script src="/FormBuilder/public/js/plugins/d3/d3.min.js"></script>
<script src="/FormBuilder/public/js/plugins/chartist/chartist.min.js"></script>
...
<script>
    $ (document).ready (function ()
    {
        ...
        c3.generate ({
            ...
            data: {
                columns: [
                    ['late', <?= count ($arrLateProjects) ?>],
                    ['on time', <?= count ($arrOnTime) ?>]
                ],
                colors: {
                    data1: '#1ab394',
                    data2: '#BABABA'
        ...
        c3.generate ({
            ...
            data: {
                columns: [
                    <?php  foreach ($arrUsersWithProjects as $user => $userCount): ?>
                        ['<?= $user ?>', <?= $userCount ?>],
                    <?php endforeach ?>
                ],
                colors: {
